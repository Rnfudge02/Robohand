...
// Use in ADC ISR:
void __isr adc_isr() {
    adc_clear_fifo_flags();
    process_adc_samples(adc_buffer); // Your data handler
}

void check_dma_perf() {
    uint32_t cycles = dma_hw->ch[pwm_dma_chan].al1_read_addr_trigger;
...
void read_mpu6050_dma() {
    ...
    dma_channel_set_write_addr(i2c_tx_dma_chan, &reg);
    dma_channel_set_trans_count(i2c_tx_dma_chan, 1, true);
    
    // 2. Read 14 bytes via DMA
    dma_channel_set_read_addr(i2c_rx_dma_chan, mpu_buffer);
