...
class RoboHandConfigurator:
    ...
    def _parse_header_file(self, file_path):
        ...
        try:
            ...
            for i, line in enumerate(lines):
                ...
                define_match = re.match(str_search, line)
                if define_match:
            ...
            for i, (line_num, line) in enumerate(define_lines):
                ...
                if define_match:
                    ...
                    value, description = self._extract_description(raw_value, lines, line, line_num)
    ...
    def _extract_description(self, raw_value, lines, line, line_num):
        ...
        if not description and line_num > 0:
            self._extract_line_above(description, lines, line_num)
                    
        return value, description
    
    def _extract_line_above(self, description, lines, line_num):
    ...
    def _create_macro_ui(self):
        ...
        for category, macros in grouped_macros.items():
            ...
            for i, macro in enumerate(macros):
                ...
                self.macro_var_map[macro.name] = self._make_macro_widget(frame, macro)
    ...
    def _make_macro_widget(self, frame, macro):
    ...
    def _save_configuration(self):
        ...
        # Prepare the macro for export
        files_to_update = self._prepare_export(macro, files_to_update)
        ...
        for file_path, updates in files_to_update.items():
            ...
            updated_files, backups = self._update_file(updated_files, backups, file_path, updates)
    ...
    def _prepare_export(self, macro, files_to_update):
        """
        @brief Format the entry and add to list if modified
        @details Retrieves, formats, and tags for updating if modified
        """
    ...
    def _update_file(self, updated_files, backups, file_path, updates):
        """
        @brief Save the configuration changes back to the header files.
        @details Creates timestamped backups of modified files, and applies changes.
        """
        ...
        try:
            ...
            for macro, new_value in sorted(updates, key=lambda x: x[0].line_number, reverse=True):
                ...
                if define_match:
                    ...
                    if '//' in after_name:
                        _, comment_part = after_name.split('//', 1)
