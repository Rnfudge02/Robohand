...
class RoboHandGUI:
    ...
    def create_connection_frame(self):
        ...
        self.status_labels = {}
        status_items = [
            conn_stat, "Core 0 Load", "Core 1 Load", 
    ...
    def toggle_connection(self):
        ...
        if not self.serial.is_connected():
            ...
            if self.serial.connect(port, baud):
                ...
                self.status_labels[conn_stat].config(text="Connected")
                ...
                self.status_labels[conn_stat].config(text="Connection Failed")
        else:
            # Disconnect
            self.serial.disconnect()
            self.connect_button.config(text="Connect")
            self.status_labels[conn_stat].config(text="Disconnected")
    ...
    def send_manual_command(self):
        ...
        if cmd and self.send_command(cmd):
    ...
    def add_sensor_point(self, sensor_type, axis, value):
        ...
        if sensor_type == 'accel' or sensor_type == 'gyro' or sensor_type == 'mag' or sensor_type == 'adc':
